...
func _ready():
	# Это ВАЖНО: чтобы эффекты работали при Engine.time_scale = 0
	process_mode = Node.PROCESS_MODE_ALWAYS
	add_child(fx_layer)
	fx_layer.layer = 128
	
	# Настройка радуги (снизу)
...
func hit_stop(duration: float):
	...
	# Ждем реальное время (четвертый аргумент true)
...
# НОВАЯ ФУНКЦИЯ ЗВУКА: без блокировки, чтобы звук был всегда
func play_ultra_sound(stream: AudioStream):
	...
	new_player.volume_db = -5.0 # 10 было слишком громко, поставил -5
	new_player.pitch_scale = randf_range(0.9, 1.1)
	new_player.process_mode = Node.PROCESS_MODE_ALWAYS 
	
	add_child(new_player)
	new_player.play()
	
	# Удаляем плеер только когда он закончит играть
